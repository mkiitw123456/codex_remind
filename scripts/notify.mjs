...
try{
  ...
  for(let attempt=0;attempt<3;attempt++){
    try{const r=await fetch(url,{method:'POST',headers:{Authorization:`Bearer ${config.notifyToken}`,'Content-Type':'application/json'},body:JSON.stringify(event),signal:AbortSignal.timeout(8000)});if(!r.ok)throw new Error(`HTTP ${r.status}`);if(hook)console.log('{}');else console.log('通知已送出');break;}
